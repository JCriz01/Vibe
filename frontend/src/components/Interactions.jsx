...
const Interactions = ({ post: post_ }) => {
  console.log("interaction obj is: ", post_);
  const user = useUserStore((state) => state.user);
  const userToken = useAtomValue(userAtom);
  const showToast = useShowToast();

  const [post, setPost] = useState(post_); //maybe remove this, redundant?
  ...
  const [liked, setLiked] = useState(
    post_.likes.some((like) => like.userId === user.id)
  );
  console.log(liked);
  ...
  const handleLikes = async () => {
    ...
    try {
      const res = await fetch(`${domainUrl}/api/posts/like/${post.id}`, {
        credentials: "include",
        method: "PUT",
        headers: {
  ...
  const handleReply = async () => {
    ...
    try {
      const res = await fetch(`${domainUrl}/api/posts/reply/${post.id}`, {
        credentials: "include",
        method: "PUT",
        headers: {
          "Content-Type": "application/json",
